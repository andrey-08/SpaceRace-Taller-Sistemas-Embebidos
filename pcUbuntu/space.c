			  	...
			  	if(player == 1){ // PARA MAESTRO 
			  		...
			  		while(1){
						...
						if (turno_player == 0){
							...
							n = read(serial_port, &buffer, sizeof(buffer));
							ch = getch(); 
							if(buffer[0] != '0' && (ch != KEY_UP || ch != KEY_DOWN)){
								turno_player == 2;
							}
							else if(buffer[0] == '0' && (ch == KEY_UP || ch == KEY_DOWN)){
								turno_player == 1;
							}
							mvprintw(46, 0, "Read %i bytes. Received message: %s\n", turno_player, buffer);
						}
...
void deserial_data2(){
	aste_cont = buff_esclavo[0];
	jugador[1].y = buff_esclavo[1] - '0';
	flagTime = buff_esclavo[2];
	turno_player = buff_esclavo[3] - '0';
	jugador[0].y = buff_esclavo[4] - '0';
	score[1] = buff_esclavo[5] - '0';
	score[0] = buff_esclavo[6] - '0';
	flag_end = buff_esclavo[7];
